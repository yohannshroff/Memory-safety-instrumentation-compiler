...
| R6 | Overhead measurement left to the last minute (graded objective) | Med | Med | Milestone 5 has its own script + `docs/overhead_results.md`; violation-free benchmarks reserved in `benchmarks/overhead/` | Open — scheduled for Review 3 prep, see `docs/review2_timeline.md` |
| R7 | Uneven contribution / integration stalls before Review 2 | Med | High | Module ownership matches Review 1 slide ownership; `memsafec` + `run_all.sh` are the integration contract; weekly sync | Open — tracked, see `docs/contribution_matrix.md` |
